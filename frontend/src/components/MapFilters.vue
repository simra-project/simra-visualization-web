...
<!--            </b-field>-->
            ...
            <b-field :label="$t('filters.weekday')">
                <b-select v-model="filterRideWeekday" @change.native="ridesChanged">
                    <option :value="null">{{ $t('filters.anyWeekday') }}</option>
                    <option v-for="weekday in weekdays" :value="weekday[0]">{{ $t(weekday[1]) }}</option>
                </b-select>
            </b-field>

            <b-field :label="$t('filters.timeOfDay')">
        ...
        <template v-else-if="viewMode === 1 || viewMode === 3">
            <b-field :label="$t('filters.scaryOrRegularIncidents')" style="margin-bottom: 0.5rem;">
                <b-checkbox v-model="filterIncidentScary" @change.native="incidentsChanged">{{ $t('filters.scaryIncidents') }}</b-checkbox>
            </b-field>
            ...
            <b-field :type="{ 'is-danger': !filterIncidentScary && !filterIncidentRegular }"
                ...
                <b-checkbox v-model="filterIncidentRegular" @change.native="incidentsChanged">{{ $t('filters.regularIncidents') }}</b-checkbox>
            </b-field>

            <b-field :label="$t('filters.incidentType')">
                <MapFiltersMultiSelect v-model="filterIncidentTypes" @input="incidentsChanged" :options="incidentTypes()" :empty-label="$t('filters.incidentTypeAny')"/>
            </b-field>

            <b-field :label="$t('filters.incidentParticipant')">
                <MapFiltersMultiSelect v-model="filterIncidentParticipants" @input="incidentsChanged" :options="incidentParticipants()" :empty-label="$t('filters.incidentParticipantAny')"/>
            </b-field>
...
export default {
    ...
    data() {
        return {
            ...
            weekdays: [
                ["Mon", "filters.weekdays.monday"],
                ["Tue", "filters.weekdays.tuesday"],
                ["Wed", "filters.weekdays.wednesday"],
                ["Thu", "filters.weekdays.thursday"],
                ["Fri", "filters.weekdays.friday"],
                ["Sat", "filters.weekdays.saturday"],
                ["Sun", "filters.weekdays.sunday"]
            ],
            timerRides: null,
            timerIncidents: null,
        }
